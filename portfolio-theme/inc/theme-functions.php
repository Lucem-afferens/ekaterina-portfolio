...
function ekaterina_handle_request_form() {
    ...
    // Валидация номера телефона (серверная проверка для безопасности)
    $phone_cleaned = preg_replace( '/\D/', '', $phone ); // Убираем все нецифровые символы
    
    // Если начинается с 8, заменяем на 7
    if ( substr( $phone_cleaned, 0, 1 ) === '8' ) {
        $phone_cleaned = '7' . substr( $phone_cleaned, 1 );
    }
    
    // Проверяем формат российского номера: должен начинаться с 7 и содержать 11 цифр
    if ( ! preg_match( '/^7\d{10}$/', $phone_cleaned ) ) {
        wp_send_json_error( array( 'message' => 'Неверный формат номера телефона. Пожалуйста, введите корректный российский номер в формате: +7 (XXX) XXX-XX-XX' ) );
    }
    
    // Проверяем код оператора (вторая цифра после 7 должна быть 3, 4, 5, 6, 7, 8 или 9)
    $operator_code = substr( $phone_cleaned, 1, 1 );
    if ( ! in_array( $operator_code, array( '3', '4', '5', '6', '7', '8', '9' ), true ) ) {
        wp_send_json_error( array( 'message' => 'Неверный код оператора в номере телефона. Пожалуйста, проверьте номер.' ) );
    }
    
    // Нормализуем номер телефона для сохранения (+7 и 10 цифр)
    $phone_normalized = '+7' . substr( $phone_cleaned, 1 );
    
    // Форматируем номер телефона для читаемого отображения (+7 (XXX) XXX-XX-XX)
    $phone_digits = substr( $phone_cleaned, 1 ); // 10 цифр без 7
    $phone_formatted = '+7 (' . substr( $phone_digits, 0, 3 ) . ') ' . substr( $phone_digits, 3, 3 ) . '-' . substr( $phone_digits, 6, 2 ) . '-' . substr( $phone_digits, 8, 2 );
    ...
    $email_body .= "Имя: $name\n";
    $email_body .= "Телефон: $phone_formatted\n";
    if ( ! empty( $email ) ) {
    ...
    $telegram_message .= "<b>Телефон:</b> " . esc_html( $phone_formatted ) . "\n";
    if ( ! empty( $email ) ) {
